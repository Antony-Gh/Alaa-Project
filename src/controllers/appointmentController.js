...
const createAppointment = asyncHandler(async (req, res) => {
  ...
  if (conflictCheck.count > 0) {
    return ResponseHandler.error(
      res,
      req.t('appointment.location_conflict'),
      409,
      'LOCATION_CONFLICT'
    );
  }
...
const createRecurringAppointments = async (parentAppointmentId, pattern) => {
  ...
  for (const instance of instances) {
    ...
    const parentAppointment = await dbManager.get(
    ...
    await dbManager.run(
      ...
      [
        instanceId,
        parentAppointment.employee_name,
        parentAppointment.employee_id,
        parentAppointment.user_id,
        parentAppointment.department_id,
        parentAppointment.location_id,
        parentAppointment.title,
        parentAppointment.description,
        instance.date,
        instance.time,
        parentAppointment.duration,
        parentAppointment.priority,
        parentAppointment.tags,
        parentAppointmentId,
      ]
    );
  }
...
const generateRecurringInstances = pattern => {
  ...
  const end = pattern.endDate
    ? moment(pattern.endDate)
    : moment().add(1, 'year');
...
const updateAppointmentStatus = asyncHandler(async (req, res) => {
  ...
  return ResponseHandler.success(
    res,
    updatedAppointment,
    req.t('appointment.status_updated')
  );
});
...
const deleteAppointment = asyncHandler(async (req, res) => {
  ...
  // Delete appointment
  const result = await dbManager.run('DELETE FROM appointments WHERE id = ?', [
    id,
  ]);
...
const getAppointmentStats = asyncHandler(async (req, res) => {
  ...
  return ResponseHandler.success(
    res,
    result,
    req.t('appointment.stats_fetched')
  );
});
