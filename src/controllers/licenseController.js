...
const getLicenseInfo = asyncHandler(async (req, res) => {
  ...
  return ResponseHandler.success(res, sanitizedInfo, 'License information retrieved successfully');
});
...
const activateLicense = asyncHandler(async (req, res) => {
  ...
  if (!licenseKey) {
    return ResponseHandler.error(res, 'License key is required', 400, 'LICENSE_KEY_REQUIRED');
  }
  ...
  if (!result.success) {
    ...
    return ResponseHandler.error(res, result.error || 'License activation failed', 400, 'LICENSE_ACTIVATION_FAILED');
  }
...
const checkFeatureAccess = asyncHandler(async (req, res) => {
  ...
  if (!featureName) {
    return ResponseHandler.error(res, 'Feature name is required', 400, 'FEATURE_NAME_REQUIRED');
  }
  ...
  }, hasAccess ? 'Feature is accessible' : 'Feature is not accessible with current license');
});
