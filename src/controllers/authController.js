...
const validateUsername = (username) => {
    ...
    if (username.length < 6) {
        throw new ValidationError('Username must be at least 6 characters', 'auth.username_too_short');
    }
    ...
    if (!englishRegex.test(username)) {
        throw new ValidationError('Username must contain only English letters, numbers, and underscores', 'auth.username_english_only');
    }
};
...
const validatePassword = (password) => {
    ...
    if (password.length < minLength) {
        throw new ValidationError('Password must be at least 8 characters long', 'auth.password_too_short');
    }
    
    if (!hasUpperCase) {
        throw new ValidationError('Password must contain at least one uppercase letter', 'auth.password_no_uppercase');
    }
    
    if (!hasLowerCase) {
        throw new ValidationError('Password must contain at least one lowercase letter', 'auth.password_no_lowercase');
    }
    
    if (!hasNumbers) {
        throw new ValidationError('Password must contain at least one number', 'auth.password_no_number');
    }
    
    if (!hasSpecialChar) {
        throw new ValidationError('Password must contain at least one special character', 'auth.password_no_special');
    }
};

const validatePasswordConfirmation = (password, confirmPassword) => {
    if (!confirmPassword || confirmPassword.trim() === '') {
        throw new ValidationError('Password confirmation is required', 'auth.confirm_password_required');
    }
    
    if (password !== confirmPassword) {
        throw new ValidationError('Passwords do not match', 'auth.passwords_not_match');
    }
};
...
const login = asyncHandler(async (req, res) => {
    ...
    if (!user) {
        ...
        const authError = new AuthenticationError('Invalid username or password', 'auth.invalid_credentials');
...
const register = asyncHandler(async (req, res) => {
    const { username, password, confirmPassword, email, role, department_id } = req.body;
    ...
    if (existingUser) {
        throw new AuthenticationError('Username already exists', 'auth.username_exists');
    }
    ...
    if (!result || !result.id) {
        logger.error('Failed to insert new user', { username, email, role, result });
    ...
    if (!newUser) {
        logger.error('Failed to retrieve newly created user', { userId: result.id });
...
const changePassword = asyncHandler(async (req, res) => {
    ...
    if (!isCurrentPasswordValid) {
        throw new AuthenticationError('Incorrect password', 'auth.password_incorrect');
    }
