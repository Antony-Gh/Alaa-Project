...
const getLocationById = asyncHandler(async (req, res) => {
  ...
  const location = await dbManager.get('SELECT * FROM locations WHERE id = ?', [
    id,
  ]);
  if (!location) {
...
const createLocation = asyncHandler(async (req, res) => {
  ...
  const existingLocation = await dbManager.get(
  ...
  const newLocation = await dbManager.get(
...
const updateLocation = asyncHandler(async (req, res) => {
  ...
  const updatedLocation = await dbManager.get(
    'SELECT * FROM locations WHERE id = ?',
    [id]
  );
...
const deleteLocation = asyncHandler(async (req, res) => {
  ...
  const result = await dbManager.run('DELETE FROM locations WHERE id = ?', [
    id,
  ]);
...
const getLocationAvailability = asyncHandler(async (req, res) => {
  ...
  if (!date || !time) {
    return ResponseHandler.error(
      res,
      req.t('location.missing_parameters'),
      400,
      'MISSING_PARAMETERS'
    );
  }
  ...
  return ResponseHandler.success(
    res,
    availability,
    req.t('location.availability_fetched')
  );
});
