...
class EmailService {
    ...
    async loadTemplates() {
        ...
        try {
            ...
            logger.warn('⚠️ Could not load email templates', { error: error.message });
        }
    }
    ...
    async sendAppointmentConfirmation(appointment, user) {
        ...
        const html = this.renderTemplate('appointment-confirmation', {
    ...
    async sendAppointmentReminder(appointment, user) {
        ...
        const html = this.renderTemplate('appointment-reminder', {
            appointment: appointment,
            user: user,
            systemName: this.t('system_name', lang)
        }, lang);
    ...
    async sendStatusUpdate(appointment, user, oldStatus, newStatus) {
        const lang = user.language || 'ar';
        const subject = this.t('email.status_update_subject', lang, { newStatus: this.t(`status.${newStatus}`, lang) });
        const html = this.renderTemplate('status-update', {
            appointment: appointment,
            user: user,
    ...
    renderTemplate(templateName, data) {
        let template = this.templates[templateName] || this.getDefaultTemplate(templateName);
        ...
        Object.keys(data).forEach(key => {
            ...
            if (typeof data[key] === 'object') {
                Object.keys(data[key]).forEach(subKey => {
                    ...
                    template = template.replace(new RegExp(subPlaceholder, 'g'), data[key][subKey] || '');
                });
            } else {
