...
async function cleanupDatabase() {
  ...
  try {
    ...
    // Get all departments
    const allDepartments = await dbManager.query('SELECT * FROM departments ORDER BY id');
    ...
    const itDept2 = allDepartments.find(d => d.name === 'قسم تكنولوجيا المعلومات');
    ...
    for (const { name, departments } of duplicates) {
      ...
      console.log(`  Deleting department IDs: ${deleteDepts.map(d => d.id).join(', ')}`);
      ...
      for (const deleteDept of deleteDepts) {
        ...
        await dbManager.run('DELETE FROM departments WHERE id = ?', [deleteDept.id]);
      }
    }
    
    // Verify cleanup
    const finalDepartments = await dbManager.query('SELECT * FROM departments ORDER BY id');
