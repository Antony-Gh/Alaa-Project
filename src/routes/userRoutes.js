...
router.put('/profile', authenticateToken, generalLimiter, async (req, res) => {
    try {
        ...
        if (email) {
            ...
            if (existingUser) {
                return ResponseHandler.error(res, 'البريد الإلكتروني مستخدم بالفعل', 400);
            }
        }
...
// Change password
router.put('/change-password', authenticateToken, generalLimiter, async (req, res) => {
    try {
        const { current_password, new_password } = req.body;

        if (!current_password || !new_password) {
            return ResponseHandler.error(res, 'كلمة المرور الحالية والجديدة مطلوبتان', 400);
        }
...
router.get('/', authenticateToken, requireAdmin, generalLimiter, async (req, res) => {
    try {
        const { page = 1, limit = 20, role, department_id, is_active, search } = req.query;
        ...
        return ResponseHandler.success(res, { users, pagination }, 'تم جلب المستخدمين بنجاح');
    } catch (error) {
...
router.get('/:id', authenticateToken, requireAdmin, generalLimiter, async (req, res) => {
    try {
        const { id } = req.params;
...
router.put('/:id', authenticateToken, requireAdmin, generalLimiter, async (req, res) => {
    try {
        const { id } = req.params;
        const { full_name, email, phone, role, department_id, is_active, email_verified } = req.body;

        // Check if user exists
        const existingUser = await dbManager.get('SELECT id FROM users WHERE id = ?', [id]);
        if (!existingUser) {
            return ResponseHandler.error(res, 'المستخدم غير موجود', 404);
        }
...
router.delete('/:id', authenticateToken, requireAdmin, generalLimiter, async (req, res) => {
    try {
        const { id } = req.params;
...
// Get user statistics
router.get('/stats/overview', authenticateToken, requireAdmin, generalLimiter, async (req, res) => {
    try {
        ...
        return ResponseHandler.success(res, stats, 'تم جلب إحصائيات المستخدمين بنجاح');
    } catch (error) {
