...
router.get('/', authenticateToken, generalLimiter, async (req, res) => {
    try {
        ...
        return ResponseHandler.success(res, { notifications, pagination }, 'تم جلب الإشعارات بنجاح');
    } catch (error) {
...
router.put('/read-all', authenticateToken, generalLimiter, async (req, res) => {
    try {
        ...
        return ResponseHandler.success(res, { updatedCount: result.changes }, 'تم تحديث جميع الإشعارات بنجاح');
    } catch (error) {
...
router.get('/stats', authenticateToken, generalLimiter, async (req, res) => {
    try {
        ...
        return ResponseHandler.success(res, stats, 'تم جلب إحصائيات الإشعارات بنجاح');
    } catch (error) {
...
router.post('/test', authenticateToken, generalLimiter, async (req, res) => {
    try {
        ...
        return ResponseHandler.success(res, { notificationId: notificationId.lastID }, 'تم إرسال الإشعار التجريبي بنجاح');
    } catch (error) {
...
router.get('/preferences', authenticateToken, generalLimiter, async (req, res) => {
    try {
        const user = await dbManager.get('SELECT preferences FROM users WHERE id = ?', [req.user.id]);
        ...
        if (user.preferences) {
            try {
                ...
                logger.warn('⚠️ Invalid user preferences JSON', { userId: req.user.id });
            }
        }

        return ResponseHandler.success(res, preferences, 'تم جلب تفضيلات الإشعارات بنجاح');
    } catch (error) {
...
router.put('/preferences', authenticateToken, generalLimiter, async (req, res) => {
    try {
        const { email, push, sms, in_app, appointment_reminders, status_updates, system_notifications } = req.body;
        ...
        return ResponseHandler.success(res, preferences, 'تم تحديث تفضيلات الإشعارات بنجاح');
    } catch (error) {
