...
async function testAPI() {
  ...
  try {
    ...
    console.log(
      `   ✅ Departments: ${departments.status} - Found ${departments.data.length || 0} departments\n`
    );
    ...
    console.log(
      `   ✅ Locations: ${locations.status} - Found ${locations.data.length || 0} locations\n`
    );
    ...
    console.log(
      `   ✅ Registration: ${register.status} - ${register.data.message}\n`
    );
    ...
    if (login.status === 200) {
      ...
      const appointment = await makeRequestWithAuth(
        '/api/appointments',
        'POST',
        appointmentData,
        token
      ...
      console.log(
        `   ✅ Appointment Creation: ${appointment.status} - ${appointment.data.message}\n`
      );

      // Test getting appointments
      console.log('7. Testing get appointments...');
      const appointments = await makeRequestWithAuth(
        '/api/appointments',
        'GET',
        null,
        token
      ...
      console.log(
        `   ✅ Get Appointments: ${appointments.status} - Found ${appointments.data.data?.length || 0} appointments\n`
      );

      // Test getting stats
      console.log('8. Testing get stats...');
      const stats = await makeRequestWithAuth(
    ...
    console.log(
      `   ✅ Admin Login: ${adminLogin.status} - ${adminLogin.data.message}\n`
    );
